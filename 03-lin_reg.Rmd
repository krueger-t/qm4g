...
Third, we solve Equation \@ref(eq:insert2) for $\hat\beta_1$:
...
Having estimates for the regression parameters, classical statistics then asks whether these estimates are "statistically significant" or could have arisen by chance from the (assumed) random process of sampling the data.^[Note, we here follow a classical statistical exposition bearing in mind that the concept of significance is not very useful and potentially dangerous [@amrhein2019]. Still, p-values can be used to judge whether results are compatible with unimportant effects.] We do this via **Analysis of Variance (ANOVA)**, which begins by constructing the ANOVA table (Table \@ref(tab:anova)). This is often done in the background in software like R and not actually looked at that much.
...
In the second column of Table \@ref(tab:anova), $SSY=\sum_{i=1}^{n}\left(y_i-\bar{y}\right)^2$ is a measure of the total variance of the data, i.e. how much the data points are varying around the overall mean (Figure \@ref(fig:ssysse), left).

$SSE=\sum_{i=1}^{n}\left(\epsilon_i\right)^2=\sum_{i=1}^{n}\left(y_i-\left(\beta_0+\beta_1 \cdot x_i\right)\right)^2$ is a measure of the error variance, i.e. how much the data points are varying around the regression line (Figure \@ref(fig:ssysse), right). This is the variance not explained by the model.
...
If these assumptions are not met then we can resort to data transformation, weighted regression or Generalised Linear Models, which we will cover in chapter \@ref(glms).
