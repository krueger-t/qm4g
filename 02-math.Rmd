...
```{r derivative, echo=FALSE, fig.align='center', fig.cap='Left: Tangent line of function $f(x)$. Centre: Secant line of function $f(x)$ between point $f\\left(x_0\\right)$ and point $f\\left(x_0+h\\right)$; the horizontal distance between these two points is $\\Delta x$ and the vertical distance is $\\Delta f(x)$. Right: Set of secant lines of function $f(x)$ between point $f\\left(x_0\\right)$ and point $f\\left(x_0+h\\right)$ for progressively decreasing increments $h$. <br><small>By derivative work: Pbroks13 (talk)Tangent-calculus.png: Rhythm - Tangent-calculus.png, CC BY-SA 3.0, https://commons.wikimedia.org/w/index.php?curid=4369975; Pbroks13 (talk)Secant-calculus.png: Shizhao - Secant-calculus.png, CC BY-SA 3.0, https://commons.wikimedia.org/w/index.php?curid=4369993; Pbroks13 (talk)Lim-secant.png: The original uploader was CSTAR at English Wikipedia. - Lim-secant.png, CC BY-SA 3.0, https://commons.wikimedia.org/w/index.php?curid=4370022.</small>', fig.show='hold', out.width='33%'}
knitr::include_graphics(c('figs/Tangent-calculus.svg','figs/Secant-calculus.svg','figs/Lim-secant.svg'))
...
# calculate intercept of tangent with y-axis
# yval = slp*x + int
int <- yval - slp*0.5


# calculate intercept of secant with y-axis, for given slope = 1.5
# yval = slp*x + int
int2 <- yval - 1.5*0.5
int2

# calculate intermediate secant lines:
int3 <- yval - 1.4*0.5
int4 <- yval - 1.3*0.5


par(mfrow=c(1,3))
plot(fx, pch = 19, type = "l", col = "darkgreen")
abline(a = int, b = slp, col = "purple")

plot(fx, pch = 19, type = "l", col = "darkgreen")
abline(a = int2, b = 1.5, col = "purple")

plot(fx, pch = 19, type = "l", col = "darkgreen")
abline(a = int2, b = 1.5, col = "purple")
abline(a = int3, b = 1.4, col = "purple")
abline(a = int4, b = 1.3, col = "purple")
...
#p + geom_text(data = annotation, aes(x=x, y=y, label=label), colour = "black", size = 7)
# doesn't work, simply returns NULL. Taken from https://www.r-graph-gallery.com/233-add-annotations-on-ggplot2-chart.html
```
