...
Centring and standardisation are used to transform different datasets onto the same scale. We will need this for the multivariate methods in later sessions (Chapter \@ref(multivariate)) or when comparing the effects of different predictors on a response variable (Chapter \@ref(multiplelinreg)).

**Centring** means subtracting from every data point $y$ the overall mean of the data $\bar{y}$:
$$\begin{equation}
y^*=y-\bar{y}
(\#eq:centring)
\end{equation}$$
...
```{r rawcentred, echo=TRUE, fig.align='center', fig.cap='Histogram of data $y$ (left) and centred data $y^*$ (right). The vertical line represents the mean.', fig.show='hold', out.width='50%'}
...
**Standardisation** means subtracting from every data point $y$ the overall mean of the data $\bar{y}$ and additionally dividing by the standard deviation $s_y$:
$$\begin{equation}
y^*=\frac{y-\bar{y}}{s_y}
(\#eq:standardisation)
\end{equation}$$
...
```{r rawstandardised, echo=TRUE, fig.align='center', fig.cap='Histogram of data $y$ (left) and standardised data $y^*$ (right). The vertical line represents the mean.', fig.show='hold', out.width='50%'}
# histogram, raw
hist(y, freq = FALSE, xlim = c(-10,10), ylim = c(0, 0.4), main = "", 
     xlab = "y", ylab = "relative frequency")
lines(c(ybar, ybar), c(0,0.4), lwd = 3)
...
text(0.3, -0.1, expression(x[0]), xpd = TRUE)
text(xval2, -0.1, expression(x[0]+h), xpd = TRUE)
text(xval3, -0.1, expression(x[0]+h_1), xpd = TRUE)
text(xval4, -0.1, expression(x[0]+h_2), xpd = TRUE)
text(0, yval+0.05, expression(f(x[0])), xpd = TRUE, adj = 0)
text(0, yval2+0.05, expression(f(x[0]+h)), xpd = TRUE, adj = 0)
text(0, yval3+0.05, expression(f(x[0]+h_1)), xpd = TRUE, adj = 0)
text(0, yval4+0.05, expression(f(x[0]+h_2)), xpd = TRUE, adj = 0)
```

Mathematically, the slope amounts to the limiting value of the ratio of the (vertical) increment of the function, $\Delta f(x)$, and the (horizontal) increment of $x$, $\Delta x$, for $\Delta x$ approaching zero, i.e. $\Delta x \to 0$:
$$\begin{equation}
...
The multiplication of two matrices is the only operation that is a bit complicated at first. It may be best to consider an example to work out the rules:
