...
(\#eq:exp)
\end{equation}$$
This is an **exponential function** with **base** 2 and **exponent** $t$. Figure \@ref(fig:explog) shows two plots of this function.
...
Note, programming often uses a different notation, which is followed in this course:
$$\begin{equation}
ln()=log()
(\#eq:lnlog)
\end{equation}$$
...
Centring and standardisation are used to transform different datasets onto the same scale. Note, for these transformations to be sensible the data $y$ need to follow (approximately) a normal distribution. We will need this for the multivariate methods in sessions (Chapter \@ref(multivariate)).
...
(\#eq:centring)
\end{equation}$$
This yields new data points $y^*$ and new mean $\bar{y^*}=0$ while the standard deviation of the transformed data remains the same: $s_{y^*}=s_y$. Centring thus shifts the data histogram to be centred on zero (Figure \@ref(fig:rawcentred)).
...
# draw random sample of y~N(1,2) of size 1000
...
**Standardisation** means subtracting from every data point $y$ the overall mean of the dataset $\bar{y}$ and dividing by the standard deviation $s_y$:
$$\begin{equation}
y^*=\frac{y-\bar{y}}{s_y}
(\#eq:standardisation)
\end{equation}$$
This yields new data points $y^*$, new mean $\bar{y^*}=0$ and new standard deviation $s_{y^*}=1$. Standardisation thus shifts the data histogram to be centred on zero and expands or contracts it to have unit standard deviation, i.e. transforming it to the scale of the standard normal distribution (Figure \@ref(fig:rawstandardised)).
...
If $y=f(t)=t^a$ then $\frac{dy}{dt}=a \cdot t^{a-1}$, i.e. multiplying the function with the exponent and reducing the exponent by one.
...
**Addition/subtraction of two matrices** also happens element-wise:
...
The result of a matrix multiplication has as many rows as the 1st matrix and as many columns as the 2nd (in our example the matrices are square). For this to work, the number of columns of the 1st matrix must match the number of rows of the 2nd matrix.
...
This matrix multiplied with itself, $\mathbf{A} \cdot \mathbf{A}$, is different to the same matrix multiplied with its inverse $\mathbf{A} \cdot \mathbf{A}'$:
...
This matrix is symmetrical, i.e. mirrored along its diagonal. Diagonal elements are so called **sums of squares**, off-diagonal elements are so called **cross-products**. This will be useful later on when working with variance-covariance matrices in Chapter \@ref(multivariate).

### Matrix division, inverse of a matrix, identity matrix

**Division of two matrices** means multiplication of one matrix with the inverse of the other:

$$\frac{\mathbf{A}}{\mathbf{B}} = \mathbf{A} \cdot \mathbf{B}^{-1}$$

The **inverse of matrix** is found so that the following equation holds:

$$\mathbf{A} \cdot \mathbf{A}^{-1} = \mathbf{A}^{-1} \cdot \mathbf{A} = \mathbf{I}$$

With $\mathbf{I}$ being the **identity matrix**, where diagonal elements are 1 and off-diagnal elements are 0:
...
Only matrices with determinants larger zero can be inverted!
...
Consider the following vectors and matrix:
